...
class Metro : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun getTimes() {
        val url = "https://tdx.transportdata.tw/api/basic/v2/Rail/Metro/StationTimeTable/TRTC?%24format=JSON&%24filter=ServiceDay%2FMonday%20eq%20true&%24orderby=StationID%2CDirection%2CDestinationStaionID"
        val req = Request.Builder().url(url).addHeader("authorization", "Bearer "+Main.Pubdata.token).build()
    ...
    private fun getMetro() {
        var url = "https://tdx.transportdata.tw/api/basic/v2/Rail/Metro/LiveBoard/TRTC?%24format=JSON&%24orderby=StationID%2CDestinationStaionID&%24select=StationID%2CTripHeadSign"
        if (line.isNotEmpty())
        ...
        OkHttpClient().newCall(req).enqueue(object: Callback {
            override fun onResponse(call: Call, response: Response) {
                val tdx = Gson().fromJson(response.body?.string(), Array<Arrival>::class.java)
